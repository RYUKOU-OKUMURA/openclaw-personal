...
<AccordionGroup>
  ...
  <Accordion title="Sandbox access">
    ...
    - On macOS, a Gateway-host `fs.listDir` result includes `nativeDirectoryPicker: true` for directly local admin connections. `fs.pickDirectory` (`operator.admin`) accepts an optional starting `path` and opens the host's native folder dialog. It returns `{ path }` for the selected absolute directory or `{ cancelled: true }`; it does not copy files or grant sandbox access. Remote connections, node targets, and non-macOS hosts cannot use this picker. Only one dialog can be open at a time, and it closes after two minutes or when its connection ends.
