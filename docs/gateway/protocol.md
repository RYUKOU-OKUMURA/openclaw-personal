...
<AccordionGroup>
  ...
  <Accordion title="Sandbox access">
    ...
    - `sandbox.entries.add` (`operator.admin`, `controlPlaneWrite`) supports `mode: "copy"`, `"ro"`, and `"rw"`. `copy` accepts `{ kind: "path", path }` for an absolute host file or folder, `{ kind: "upload", name, contentBase64 }` for one upload up to 16 MiB, or `{ kind: "create", name, entryKind: "file" | "directory" }` for an empty file or folder. `ro` and `rw` accept only `{ kind: "path", path }`; the Gateway resolves the canonical real host path, validates it, and creates a bind under `/mnt/shared/<name>`, adding a numeric suffix when the name conflicts with an effective mount. Path sources for `copy` are materialized in private staging with symlinks dereferenced before import. The result returns the resolved entry fields and `recreateRequired` (`false` for `copy`, `true` for a share); no full config is returned.
    - A `ro` or `rw` bind is appended to the selected agent's authored `sandbox.docker.binds`, preserving that agent's configuration. When the resolved Docker scope is `shared`, the owner is `agents.defaults.sandbox.docker.binds`, so the bind applies to agents inheriting that shared configuration. If the canonical source is outside the effective or agent workspace roots, `allowExternalSource: true` is required unless the existing `dangerouslyAllowExternalBindSources` override is already enabled. That consent can enable the existing override; blocked system, credential, and Docker socket paths remain disallowed after consent.
    - Accepting or removing a share changes the configured mounts, while an already-running container keeps its current mounts until the target sandbox is recreated. The configured state and running state can therefore differ, and removal drops that mount from a newly recreated container; other mounts can still expose the same source. There is no separate removal RPC: read a fresh config with `config.get`, then use `config.patch` with a fresh `baseHash`, the exact owning binds array in `replacePaths`, and the intended bind value removed. Preserve sibling binds and the existing external-source override; it is not cleared automatically.
    - `sandbox.recreate` (`operator.admin`, `controlPlaneWrite`) resolves the same agent, workspace, and lifecycle identity as `sandbox.explain`, then targets at most one matching Docker registry entry by exact container name, backend, and scope key. It removes that runtime now and returns `{ removed: string[], failed: [{ containerName, error }] }`; no matching entry returns empty arrays, while a removal failure is reported in `failed` in a successful best-effort response. A later sandbox use creates the current configuration; the RPC does not restart the Gateway or create a container immediately. Disabled, non-Docker, and unknown-agent requests are invalid, and registry read failures are unavailable. Removing a shared runtime interrupts users of that runtime, while source and bind files remain untouched. CLI agent-wide and browser recreation behavior remains separate.
    - All three methods use the same resolved agent/workspace context.
    ...
    - On macOS, a Gateway-host `fs.listDir` result includes `nativeDirectoryPicker: true` and `nativeFilePicker: true` for directly local admin connections. `fs.pickDirectory` and `fs.pickFile` (`operator.admin`) accept an optional starting folder `path` and open the host's native folder or single-file dialog. They return `{ path }` for the selected absolute directory or regular file, or `{ cancelled: true }`; selection does not copy files or grant sandbox access. The file dialog accepts images and other regular files without an extension filter. Remote connections, node targets, and non-macOS hosts cannot use these pickers. The two methods share one active-dialog limit, and the dialog closes after two minutes or when its connection ends.
