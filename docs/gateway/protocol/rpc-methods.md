...
<AccordionGroup>
  ...
  <Accordion title="Sandbox access">
    ...
    - On macOS, a Gateway-host `fs.listDir` result includes `nativePathPicker: true` for directly local admin connections. `fs.pickPath` (`operator.admin`) accepts an optional starting folder `path` and opens one native dialog that accepts both files and folders. It returns `{ path, kind }` with the selected absolute path and `kind: "file" | "directory"`, or `{ cancelled: true }`; selection does not copy files or grant sandbox access. The dialog accepts images and other regular files without an extension filter. Remote connections, node targets, and non-macOS hosts cannot use this picker. Only one dialog can be active, and the dialog closes after two minutes or when its connection ends.
