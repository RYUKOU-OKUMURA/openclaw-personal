...
<AccordionGroup>
  <Accordion title="Send and history semantics">
    ...
    - Search the active transcript with **⌘F** on Mac or **Ctrl+F** on Windows/Linux; Mac **Ctrl+F** remains available for native text navigation. Search includes recovered full-message text and updates when an in-flight recovery finishes. Clearing the search keeps recovered text available in the thread.
    ...
    - For local sessions with `operator.admin` access, **Work files** keeps the existing workspace browser and transcript artifacts. **Shared materials** lists configured Docker or Podman bind shares below `/mnt/shared/`; choose a share to browse and preview its files without copying or moving them. The panel shows the sandbox path and configured read-only status. Extra locations are not exposed for exec-node, cloud-worker, or remote-backend sessions.
    - **Outputs** opens the `outputs` directory under the session's effective working area, such as `/workspace/outputs` inside a Docker sandbox. Sandboxed agents receive shared-path guidance and the outputs convention in their instructions; when writable, they create the directory when saving a deliverable. Browsing does not create it. Once it exists, **Open output folder** opens it in the Gateway host's file manager, not on a remote browser's computer. Shared-material and output previews do not offer editing; existing workspace editing remains in **Work files**. These locations do not grant additional agent filesystem permissions.
