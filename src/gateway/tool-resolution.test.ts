  ...
  it.each(["rw", "ro"] as const)(
    ...
    async (workspaceAccess) => {
      ...
      const sandbox = createAgentToolsSandboxContext({
        ...
        // The host-I/O fixture models the released v2026.9.4 SDK bridge,
        // whose physical path guard remains owned by the coding factory.
        // Current descriptor-aware Docker bridges are covered by core-coding-tools.sandbox.test.ts.
        fsBridge: { ...createHostSandboxFsBridge(sandboxDir), pathMappings: undefined },
      });
      const provision = vi
