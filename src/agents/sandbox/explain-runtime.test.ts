...
async function resolveExpectedHash(snapshot: ExplainSnapshot): Promise<{
  ...
  const resources = await resolveSandboxSessionResourceMounts({
    scope: snapshot.sandboxConfig.scope,
    agentId: snapshot.report.agentId,
    sessionKey: snapshot.report.sessionKey,
  });
  const mounts = await prepareSandboxMountPlan({
...
describe("readSandboxExplainRegistry", () => {
  it("keeps an unchanged default-workspace hash fresh with uid/gid, epoch, and protected mounts", async () => {
    await withOpenClawTestState({ scenario: "minimal" }, async (state) => {
      ...
      expect(mounts.readOnlyWorkspaceSkillMounts).toEqual(
  ...
  it("keeps a child attachment mount fresh and detects its removal", async () => {
    await withOpenClawTestState({ scenario: "minimal" }, async (state) => {
      const cfg = createConfig(state);
      const sessionKey = "agent:main:subagent:attachment-hash";
      const attachmentRoot = resolveSubagentSessionAttachmentRootDir({
        agentId: "main",
        childSessionKey: sessionKey,
      });
      await fs.mkdir(attachmentRoot, { recursive: true });
      const snapshot = resolveSandboxExplainContext({ cfg, sessionKey });
      const { hash } = await resolveExpectedHash(snapshot);
      const entry = createRegistryEntry(snapshot, { configHash: hash });
      sandboxMocks.readRegistryEntry.mockResolvedValue(entry);
      await expect(readSandboxExplainRegistry(snapshot, cfg)).resolves.toMatchObject({
        stale: false,
      });
      await fs.rm(attachmentRoot, { recursive: true });
      await expect(readSandboxExplainRegistry(snapshot, cfg)).resolves.toMatchObject({
        stale: true,
      });
    });
