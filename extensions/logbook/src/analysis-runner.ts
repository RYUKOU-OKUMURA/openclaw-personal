...
export async function runLogbookBatch(params: BatchParams): Promise<void> {
  ...
  try {
    ...
    const existing = (
      await store.observationsInRange(batch.day, batch.startMs, batch.endMs)
    ...
    if (cursor < batch.endMs) {
      ...
      let sampled: Awaited<ReturnType<LogbookStore["batchImages"]>>;
      try {
        sampled = await store.batchImages(batch.id);
      } catch (error) {
        // Reading images is an attempt too: unreadable files must not bypass the retry budget.
        await store.beginBatch(batch.id, `${vision.provider}/${vision.model}`);
        throw error;
      }
      for (let offset = 0; offset < sampled.length; offset += OBSERVATION_CHUNK_SIZE) {
        ...
        const startMs = offset === 0 ? batch.startMs : chunk[0]!.frame.capturedAtMs;
        const endMs = sampled[offset + OBSERVATION_CHUNK_SIZE]?.frame.capturedAtMs ?? batch.endMs;
        if (endMs <= cursor) {
          continue;
        }
        await store.beginBatch(batch.id, `${vision.provider}/${vision.model}`);
        ...
        await store.checkpointObservations(batch, endMs, segments);
        cursor = endMs;
      }
    }
    await store.beginBatch(batch.id);
