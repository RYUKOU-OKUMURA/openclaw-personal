...
async function makeService(params: {
  ...
  const createService = () =>
    new LogbookService(resolveLogbookConfig({ captureEnabled: true, ...params.config }), {
...
describe("LogbookService capture node selection", () => {
  ...
  it("captures the live configured screen and keeps an in-flight frame on its original screen", async () => {
    ...
    const [capturedFrame] = await fixture.service.framesInRange(0, Date.now() + 1);
    const reader = await LogbookStore.open(fixture.dataDir, workerModuleUrl);
    try {
      expect(capturedFrame).toBeDefined();
      expect(await reader.frameById(capturedFrame!.id)).toMatchObject({ screenIndex: 1 });
    } finally {
      await reader.close();
    }
    expect(await fixture.service.status()).toMatchObject({ screenIndex: 0 });
    await fixture.tick();
...
describe("LogbookService screen selection", () => {
  ...
  it("rejects invalid choices and failed persistence without changing capture state", async () => {
    ...
    try {
      ...
      expect(await fixture.service.status()).toMatchObject({ screenIndex: 1, capturePaused: true });
      ...
      expect((await fixture.service.status()).capturePaused).toBe(true);
    } finally {
      await fixture.service.stop();
      rmSync(fixture.dataDir, { recursive: true, force: true });
    }
  ...
  it("waits for the active config and reports when a saved choice cannot be applied", async () => {
    ...
    try {
      ...
      await failure;
      expect(await fixture.service.status()).toMatchObject({ screenIndex: 0, capturePaused: true });
    } finally {
      await fixture.service.stop();
      rmSync(fixture.dataDir, { recursive: true, force: true });
      vi.useRealTimers();
...
describe("LogbookService capture schedule", () => {
    ...
    async (schedule, times, counts) => {
      ...
      try {
        for (const [index, time] of times.entries()) {
          ...
          expect((await fixture.service.status()).captureSchedulePaused).toBe(
    ...
    async (time) => {
      ...
      try {
        for (const hour of [0, 7, 8, 23]) {
          ...
          await fixture.tick();
          expect((await fixture.service.status()).captureSchedulePaused).toBe(true);
        }
  ...
  it("rejects invalid schedules and failed writes without removing the saved pause", async () => {
    ...
    try {
      ...
      expect((await fixture.service.status()).captureSchedule).toEqual(schedule);
    } finally {
      await fixture.service.stop();
      rmSync(fixture.dataDir, { recursive: true, force: true });
    }
  ...
  it.each([
    ...
    async (_label, textModel, expectedModel) => {
      ...
      try {
        ...
        expect((await service.status()).captureSchedulePaused).toBe(true);
        await expect(service.analyzeNow()).resolves.toEqual({ started: true });
        await vi.waitFor(async () =>
          expect((await service.status()).lastBatch?.status).toBe("done"),
        );
        expect((await service.timelineForDay("2026-08-01")).cards).toMatchObject([
          { title: "Editing a document" },
        ]);
...
describe("LogbookService text completion validation", () => {
  it("does not restore a stale standup after its source timeline changes", async () => {
    const fixture = await makeService({
      ...
      invoke: async () => framePayload,
      complete: async () => {
        const writer = await LogbookStore.open(fixture.dataDir, workerModuleUrl);
        try {
          await writer.replaceCardsInWindow("2026-08-01", 1, 1000, [
            {
              day: "2026-08-01",
              startMs: 1,
              endMs: 1000,
    ...
    try {
      ...
      const reader = await LogbookStore.open(fixture.dataDir, workerModuleUrl);
      try {
        expect(await reader.getStandup("2026-08-01")).toBeNull();
      } finally {
        await reader.close();
      }
    } finally {
...
describe("LogbookService bounded observation dispatch", () => {
  it("dispatches no more than four screenshots per vision request", async () => {
    ...
    try {
      ...
      await vi.waitFor(async () =>
        expect((await fixture.service.status()).analysisRunning).toBe(false),
      );
      expect(extractStructured.mock.calls.map(([request]) => request.input.length)).toEqual([4, 4]);
      expect((await fixture.service.status()).lastBatch?.status).toBe("done");
    } finally {
      await fixture.service.stop();
      rmSync(fixture.dataDir, { recursive: true, force: true });
      vi.useRealTimers();
...
describe("Logbook deletion admission", () => {
  it("holds new capture and analysis until asynchronous day deletion settles", async () => {
    const fixture = await makeService({
      nodes: [{ nodeId: "mac", commands: ["screen.snapshot"] }],
      invoke: async () => framePayload,
    });
    ...
    const original = Object.getOwnPropertyDescriptor(LogbookStore.prototype, "deleteDay")
      ?.value as LogbookStore["deleteDay"];
    const remove = vi.spyOn(LogbookStore.prototype, "deleteDay").mockImplementation(async function (
      this: LogbookStore,
      day: string,
    ) {
      await gate;
      return await Reflect.apply(original, this, [day]);
    });
    const deletion = fixture.service.deleteDay("2026-09-05");
    try {
      await vi.waitFor(() => expect(remove).toHaveBeenCalledOnce());
      await fixture.tick();
      expect(fixture.invoked).toEqual([]);
      await expect(fixture.service.analyzeNow()).rejects.toThrow("not running");
      release();
      await deletion;
      await fixture.tick();
      expect(fixture.invoked).toHaveLength(1);
