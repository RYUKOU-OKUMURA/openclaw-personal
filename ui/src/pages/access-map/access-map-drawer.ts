          ...
          create
            ...
            : html` <div class="access-map-selected-file">
                  ...
                  <div>
                    ...
                    ><small
                      >${
                        p.draft.kind === "directory" ? t("accessMap.folder") : t("accessMap.file")
                      }${
                        p.draft.size === undefined
                          ? nothing
                          : ` · ${(p.draft.size / 1024).toFixed(1)} KB`
                      }</small
                    >
                  </div>
                </div>
...
export function renderAccessMapPicker(p: AccessMapPickerProps) {
  ...
  >
    <section class="access-map-picker">
      ...
      ${
        p.error ? html`<p class="access-map-message is-error" role="alert">${p.error}</p>` : nothing
      }
                ...
                ${p.listing.entries.map(
                  ...
                  >
                    ${icon(
                      ...
                      entry.kind === "directory" ? icon("chevronRight") : nothing
                    }
                  </button>`,
                )}
                ${
