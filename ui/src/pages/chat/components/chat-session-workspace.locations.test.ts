...
function activeContent(state: SessionWorkspaceHost) {
  return (
    state.sessionWorkspaceState?.previews.find(
...
describe("Files location ownership", () => {
  it("scopes shared list and preview requests without artifacts or editing, while chat links keep their workspace scope", async () => {
    ...
    await vi.waitFor(() => expect(activeContent(fixture.state)?.kind).toBe("file"));
    expect(fixture.getFile).toHaveBeenLastCalledWith(sessionKey, "note.md", {
      agentId: "main",
      rootId: "shared:notes",
    });
    expect(activeContent(fixture.state)).not.toHaveProperty("edit");
    expect(activeContent(fixture.state)).toHaveProperty("previewOnly", true);
    expect(activeContent(fixture.state)).toHaveProperty(
      "draftKey",
    ...
    await vi.waitFor(() =>
      expect(activeContent(fixture.state)).toHaveProperty("content", "Workspace note"),
    );
    expect(fixture.getFile).toHaveBeenLastCalledWith(sessionKey, "note.md", { agentId: "main" });
    expect(activeContent(fixture.state)).toHaveProperty("edit");
    expect(activeContent(fixture.state)).not.toHaveProperty("previewOnly");
  });

  it("keeps shared previews read-only when the same physical file is also editable in Work files", async () => {
    const fixture = createFixture();
    await fixture.ready();
    fixture.getFile.mockResolvedValue(preview("Workspace note", "/host/work", false));
    openSessionWorkspaceFile(fixture.state, { path: "note.md" });
    await vi.waitFor(() => expect(activeContent(fixture.state)).toHaveProperty("edit"));
    await fixture.select("shared:notes");
    fixture.getFile.mockResolvedValue(preview("Workspace note", "/host/work", true));
    fixture.props().onOpenFile("note.md", "workspace");
    await vi.waitFor(() =>
      expect(activeContent(fixture.state)).toHaveProperty("previewOnly", true),
    );
    expect(activeContent(fixture.state)).not.toHaveProperty("edit");
    expect(fixture.state.sessionWorkspaceState?.previews).toHaveLength(2);
  });
  ...
  it("keeps diff file links scoped to the workspace while shared materials are selected", async () => {
    ...
    await vi.waitFor(() =>
      expect(activeContent(fixture.state)).toHaveProperty("content", "Changed workspace note"),
    );
    expect(fixture.getFile).toHaveBeenLastCalledWith(sessionKey, "note.md", { agentId: "main" });
    expect(activeContent(fixture.state)).toHaveProperty("edit");
  });

  it("keeps a late shared preview from replacing the active output tab", async () => {
    const fixture = createFixture();
    await fixture.ready();
    await fixture.select("shared:notes");
    ...
    fixture.props().onOpenFile("note.md", "workspace");
    await vi.waitFor(() =>
      expect(activeContent(fixture.state)).toHaveProperty("content", "Current output"),
    );
